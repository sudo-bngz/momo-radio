...
var CountryMap = map[string]string{
	...
	"VC": "Saint Vincent and the Grenadines", "VE": "Venezuela, Bolivarian Republic of", "VG": "Virgin Islands (British)",
	"VI": "Virgin Islands (U.S.)", "VN": "Viet Nam", "VU": "Vanuatu", "WF": "Wallis and Futuna",
	"WS": "Samoa", "YE": "Yemen", "YT": "Mayotte", "ZA": "South Africa", "ZM": "Zambia",
	"ZW": "Zimbabwe",
}

// ResolveCountry returns the full name for a code, or the code if not found.
func ResolveCountry(code string) string {
	if name, found := CountryMap[code]; found {
		return name
	}
	return code
}
...
func init() {
	...
	for code, name := range CountryCode {
