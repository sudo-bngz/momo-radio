...
import { useAuthStore } from '../../../store/useAuthStore'; // ⚡️ Needed to securely fetch the formatted track
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      pendingIds.forEach(async (id) => {
        try {
          // Check the raw status from the DB
          const updated = await api.getTrack(id) as any; 
          
          if (updated.processing_status === 'completed') {
            // ⚡️ THE FIX: Ask the backend for the officially formatted track!
            // This guarantees the cover_url and artist strings are perfectly generated by Go.
            ...
            // Extract our specific track from the formatted list
