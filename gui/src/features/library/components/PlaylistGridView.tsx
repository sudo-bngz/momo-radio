...
const PlaylistMosaic = ({ tracks = [] }: { tracks: any[] }) => {
  ...
  if (rawCovers.length === 0) {
    return (
      // ⚡️ Semantic fallback background and icon color
    ...
    <Grid templateColumns="repeat(2, 1fr)" templateRows="repeat(2, 1fr)" w="full" h="full">
      {covers.map((src, i) => (
        <Box 
          ...
          borderColor="bg.panel" // ⚡️ Perfectly matches the card background in both light and dark mode
        >
...
export const PlaylistGridView: React.FC = () => {
  ...
  const handleExport = async (e: React.MouseEvent, playlist: Playlist) => {
    ...
    toaster.create({
      ...
      description: "Packaging files for M3U playlist. This may take a moment...", 
      type: "info",
      duration: 4000,
    });
        ...
        {isLoading ? (
          ...
          <SimpleGrid columns={{ base: 1, md: 2, xl: 3, "2xl": 4 }} gap={6} px={1}>
            {filteredPlaylists.map((playlist) => {
              ...
              return (
                <Flex 
                  key={playlist.id} direction="column" bg="bg.panel" borderRadius="2xl" 
                  borderWidth="1px" borderColor="border" overflow="hidden" shadow="sm" 
                ...
                >
                  ...
                  <Box position="relative" h="160px" w="full" bg="gray.50" _dark={{ bg: "whiteAlpha.100" }} flexShrink={0} overflow="hidden">
                    ...
                    <Box position="absolute" inset={0} bg="blackAlpha.500" opacity={0} _groupHover={{ opacity: 1 }} transition="opacity 0.2s" />

                    <HStack position="absolute" top={3} right={3} gap={2} opacity={0} _groupHover={{ opacity: 1 }} transition="opacity 0.2s">
                      <Button size="xs" variant="solid" bg="whiteAlpha.900" color="purple.600" _dark={{ bg: "gray.800", color: "purple.300" }} backdropFilter="blur(10px)" _hover={{ bg: "white", transform: "scale(1.05)", _dark: { bg: "gray.700" } }} onClick={(e) => handleExport(e, playlist)} title="Export to M3U">
                    ...
                    <Button 
                      position="absolute" bottom={4} right={4} size="lg" w="48px" h="48px" borderRadius="full" bg="white" color="gray.900" _dark={{ bg: "blue.500", color: "white" }} shadow="xl"
